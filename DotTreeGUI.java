...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			Dot d = new Dot(x, y);
			if (tree == null) {
			tree = new PointQuadtree<Dot>(d, 0, 0, width, height);
			}
			else {
				tree.insert(d);
			}
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		g.drawLine((int)(tree.getPoint().getX()), tree.getY1(), (int)(tree.getPoint().getX()), tree.getY2()); // vertical line 
		g.drawLine(tree.getX1(), (int)(tree.getPoint().getY()), tree.getX2(), (int)(tree.getPoint().getY())); // horizontal line 
