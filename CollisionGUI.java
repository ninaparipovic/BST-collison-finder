...
public class CollisionGUI extends DrawingGUI {
	...
	private void findColliders() {
		// Create the tree
		if(colliders == null) colliders = new ArrayList<Blob>();
		if(blobs.size() != 0) {
			PointQuadtree<Blob> tree = new PointQuadtree<Blob>(blobs.get(0), 0, 0, width, height);
			for (int i=1; i<blobs.size(); i++){ // add all but the root of the tree (first blob)
				tree.insert(blobs.get(i));
		}
		
		// For each blob, see if anybody else collided with it
		for (Blob blob: blobs) {
			List<Blob> inCircle = tree.findInCircle(blob.getX(), blob.getY(), 2*blob.getR());
			// if the list has more blobs then just the initial 
			if(inCircle.size() > 1) {
				for (Blob collider : inCircle) {
